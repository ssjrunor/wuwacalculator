...
export function buildCombinadicIndexing({
    ...
    if (Number.isInteger(lockedEchoIndex)) {
        if (lockedEchoIndex >= 0 && lockedEchoIndex < n) {
            lockedIndex = lockedEchoIndex;
        } else if (lockedEchoIndex !== -1) {
            return { totalCombos: 0 };
        }
    } else if (lockedEchoId != null) {
        for (let i = 0; i < n; i++) {
