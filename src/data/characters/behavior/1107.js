export function applyCarlottaLogic({
    ...
    if (characterState?.activeStates?.deconstruction && !mergedBuffs.__carlottaDeconstruction) {
        mergedBuffs.attribute.all.defIgnore += 18; mergedBuffs.__carlottaDeconstruction = true;
    }
    ...
    if (isToggleActive(1) && isActiveSequence(1)) {
        if (!mergedBuffs.__seq1) mergedBuffs.critRate += 12.5; mergedBuffs.__seq1 = true;
    ...
    if (isToggleActive(4) && isActiveSequence(4)) {
        if (!mergedBuffs.__seq4) {
            mergedBuffs.skillType.resonanceSkill.dmgBonus += 25; mergedBuffs.__seq4 = true; }
...
export function carlottaBuffsLogic({mergedBuffs, characterState}) {
    const state = characterState?.activeStates ?? {};
    if (state.raindrops) mergedBuffs.skillType.resonanceSkill.dmgBonus += 25;
    return { mergedBuffs }; }